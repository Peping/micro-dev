...
const shortenLongJson = jsonNode => {
	if (Array.isArray(jsonNode)) {
		if (jsonNode.length > 6) {
			return [...jsonNode.slice(0,6).map(shortenLongJson), truncatedSuffix]
		}
		return jsonNode.map(shortenLongJson)
	} else if (typeof jsonNode === "object") {
		return Object.entries(jsonNode).reduce(
			(acc, [k,v]) => ({...acc, [k]: shortenLongJson(v)}),
			{}
		);
	} else if (typeof jsonNode === "string") {
		if (jsonNode.length >= 200) {
			return jsonNode.slice(0, 200 - truncatedSuffix.length) + truncatedSuffix;
		}
		return jsonNode;
	} else {
		return jsonNode;
...
const logResponse = async ({res, end, endTime, requestIndex, chunk}) => {
	...
	try {
		...
		const processed = shortenLongJson(str)
		jsome(processed);
	} catch (err) {
